...
def preprocess_input(net_input_shape, image):
...
def preprocess_outputs(threshold, outputs):
    boxes = []
    probs = outputs[0, 0, :, 2]
...
def draw_boxes(frame, result, args, width, height, inference_time):
    ...
    for box in result[0][0]: 
        # box[1] contains class id and 1 means people (prediction class ID)
        if  box[2] >= args.prob_threshold and box[1] == 1:
...
def publish_to_mqtt(params, args, mqttclient):
    '''
    Assess how many people are present in current frame and compare with previous frame and publish it to MQTT accordingly
    '''
    # Extract the parameters into variables
    total_count = params['total_count']
    previous_count = params['previous_count']
    current_count = params['current_count']
    current_duration = params['current_duration']
    previous_duration = params['previous_duration']

    # Set the enterance duration to the previous duration
    in_duration = previous_duration

    # Check the new people whom come in the frame compared to the previous frame 
    if(current_count > previous_count):
        # presumingly assuming people won't leave the frame after having been in the frame less than the leave threshold
        if ((current_duration - previous_duration) < args.leave_threshold):
        ...
        # record the duration they enter the video to calculate their duration in frame later once they leave
            ...
            in_duration = current_duration
            # publish updated people counter
            mqttclient.publish("person", json.dumps({"count": current_count}))

    # Check for people whom left the frame compared to the previous frame
    elif(current_count < previous_count):
        # presumingly assuming people won't leave the frame after having been in the frame less than the leave threshold
        if ((current_duration - previous_duration) < args.leave_threshold):
            mqttclient.publish("person", json.dumps({"count": current_count}))
            return total_count, current_count, current_duration
        else:
            # record the time they left the frame
            out_duration = current_duration
            ...
            # increase the total counter for new people only
            total_count += 1
            mqttclient.publish("person", json.dumps({"total": total_count,"count": current_count}))

    # Check for people whom left the frame compared to previous frame
    else:
        # send notification in case a person duration exceeds 10 seconds
        if((current_duration - previous_duration) >= 11 ):
            mqttclient.publish("person/exceed_10s", json.dumps({"exceed_10s": "true"}))
...
def infer_on_stream(args, client):
    ...
    while cap.isOpened():
        ...
        p_frame = preprocess_input(net_input_shape, frame)
        ...
        if infer_network.wait() == 0:
            ...
            # Preprocess the output results
            
            # boxes = preprocess_outputs(prob_threshold, outputs)  --> didn't work

            # Draw the boxes on the video
            current_count, out_frame = draw_boxes(frame, outputs, args, width, height, inference_time)
            ...
            total_count, previous_count, previous_duration = publish_to_mqtt(params, args, client)
